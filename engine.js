...
const REWARD_FOODS={fish:{name:'小魚乾',price:15,duration:3,mood:1,bond:0,meows:[1,2],heartEvery:1},can:{name:'高級罐頭',price:30,duration:6,mood:2,bond:0,meows:[3,4],heartEvery:.75},catnip:{name:'貓薄荷零食',price:50,duration:10,mood:2,bond:1,meows:[5,7],heartEvery:.6}};
...
class World{
 ...
 yarn={x:145,y:278};
 fountain={x:6,y:294};
 refillWater(){if(this.water>=15){this.emit('note','已經滿水了，不用再補囉！');return false;}this.applyStatDelta({target:'shared',stat:'water',requestedDelta:15},{source:'REFILL_WATER'});this.checkDailyFullStats(['water','clean']);this.emit('note','飲水機已補滿，水量 15/15。');return true;}
 ...
 this.nextCritter=this.time+90+this.random()*150;
   ...
   const kind=this.random()<.5?'mouse':'ant',direction=kind==='mouse'?-1:1,follows=kind==='mouse'||this.random()<.65,target=follows?cats[Math.floor(this.random()*cats.length)]:null;
 ...
 hitMosquito(x,y){const m=this.mosquito;if(!m||Math.hypot(x-m.x,y-m.y)>15)return false;this.mosquitoPop={x:m.x,y:m.y,age:0};if(!m.coinRewarded){m.coinRewarded=true;this.grantCoins(2,'打到蚊子',this.current);}this.mosquito=null;this.nextMosquito=this.time+120+this.random()*180;this.emit('mosquito-stop');this.emit('swat','啪！打到蚊子了。');return true;}
 ...
 hitGecko(x,y){const v=this.gecko;if(!v||Math.hypot(x-v.x,y-v.y)>11)return false;v.scurryDistance=Math.min(42,(v.scurryDistance||0)+26);v.pause=0;this.emit('gecko-scurry','壁虎咻地往前爬了幾步。');return true;}
 chase(c,dt,toy){
  ...
  if(toy.level){c.windowIntent=null;c.desiredLevel=toy.level;if(c.level===toy.level){if(c.state!=='play')this.set(c,'play');if(this.playEnergy(c,dt)&&this.time-c.rewardAt>10){this.grantActiveInteractionCoins(2,'有效逗貓棒追逐',c);c.rewardAt=this.time;this.emit('toy',null,c);}}else if(c.level){this.jumpTo(c,toy.level);}else{if(c.goal!=='climb')this.climb(c,toy.level);this.move(c,dt,defs[c.id].speed*(c.id==='hoya'?2.5:1.65));}return;
  }
  ...
  else{if(c.state!=='play')this.set(c,'play');const rewardAllowed=c.energy>2&&c.anger<10&&!(c.playRestUntil>Date.now());if(rewardAllowed&&this.playEnergy(c,dt)&&this.time-c.rewardAt>10){this.grantActiveInteractionCoins(2,'有效逗貓棒追逐',c);c.rewardAt=this.time;this.emit('toy',null,c);}}
 }
 constructor(saved,random=Math.random){
 this.random=random;this.cats=[freshCat('lumi'),freshCat('hoya')];this.selected='lumi';this.coins=150;this.dailyCoinRewards=freshDailyCoinRewards();this.water=15;this.isGoodnightMode=false;this.messyObjects=Object.fromEntries(MESS_KEYS.map(key=>[key,false]));Object.defineProperties(this,{night:{get:()=>this.isGoodnightMode,set:value=>{this.isGoodnightMode=!!value;}},manualNight:{get:()=>this.isGoodnightMode,set:value=>{this.isGoodnightMode=value===true;}},plantKnocked:{get:()=>this.messyObjects.plant,set:value=>{this.messyObjects.plant=!!value;}},lampKnocked:{get:()=>this.messyObjects.lamp,set:value=>{this.messyObjects.lamp=!!value;}}});this.outdoorNight=false;this.close=null;this.events=[];this.foods=[];this.foodSerial=0;this.rewardFood=null;this.rewardSerial=0;this.toy=null;this.time=0;this.poops=0;this.paintingIndex=0;this.lastThreat='';this.lastChaosEvaluationAt=Date.now();this.nextMischief=180;this.nextBird=18;this.nextPlane=45+random()*45;this.nextWolf=80+random()*65;this.nextMeteor=60;this.nextYarnPlay=90;this.nextCritter=120;this.nextMosquito=100;this.nextGecko=180+random()*180;this.mosquito=null;this.gecko=null;
 ...
 grantCoins(amount,reason,c){const before=this.coins;this.coins=clamp(Math.floor(this.coins+(Number(amount)||0)),0,99999);const granted=this.coins-before;if(granted>0)this.emit('coin',`${reason} 🪙 +${granted}`,c);return granted;}
 grantActiveInteractionCoins(amount,reason,c,now=Date.now()){const daily=this.ensureDailyCoinRewards(now),granted=Math.min(Math.max(0,Math.floor(amount||0)),20-daily.activeInteractionCoins);if(granted>0){daily.activeInteractionCoins+=this.grantCoins(granted,reason,c);if(daily.activeInteractionCoins>=20&&!daily.activeCapNoticeShown){daily.activeCapNoticeShown=true;this.emit('note','今日互動獎勵已達上限 🪙 +20');}}return granted;}
 checkDailyFullStats(keys,{bondInteraction=false,now=Date.now()}={}){const daily=this.ensureDailyCoinRewards(now),labels={hunger:'飽足',mood:'心情',energy:'精力',bond:'親密',clean:'潔淨',water:'水量'};let total=0;for(const key of keys){if(!DAILY_FULL_STATS.includes(key)||daily.fullStats[key]||key==='bond'&&!bondInteraction)continue;const full=key==='clean'?this.cleanliness>=30:key==='water'?this.water>=15:this.cats.some(c=>c[key]>=statLimits[key][1]);if(!full)continue;daily.fullStats[key]=true;total+=this.grantCoins(5,`${labels[key]}首次滿值`,key==='clean'||key==='water'?null:this.current);}return total;}
 paintingAt(x,y){return x>=144&&x<=174&&y>=92&&y<=127;}
 selectPainting(index){index=Math.floor(index);if(!Number.isFinite(index)||index<0||index>=paintingNames.length)return false;if(index===this.paintingIndex){this.emit('note',`牆上現在就是「${paintingNames[index]}」。`);return false;}if(this.coins<PAINTING_PRICE){this.emit('note',`更換照片需要 ${PAINTING_PRICE} Coins。`);return false;}this.coins-=PAINTING_PRICE;this.paintingIndex=index;this.emit('coin',`換上像素畫：${paintingNames[index]}（-${PAINTING_PRICE} Coins）。`);return true;}
 get cleanliness(){const messPenalty=MESS_KEYS.reduce((sum,key)=>sum+(this.messyObjects[key]?MESS_PENALTIES[key]:0),0);return clamp(getCleanlinessFromPoops(this.poops)-Math.floor((15-this.water)/3)-messPenalty,0,30);}
 ...
 setMess(kind,{feedback=true,note=true}={}){if(!MESS_PENALTIES[kind]||this.messyObjects[kind]||MESS_KEYS.filter(key=>this.messyObjects[key]).length>=5)return false;const before=this.careScore(),clean=this.cleanliness;this.messyObjects[kind]=true;if(feedback)this.emitStatFeedback([{id:'shared',stat:'cleanliness',delta:this.cleanliness-clean}],before,this.careScore(),`MISCHIEF_${kind.toUpperCase()}`);if(note)this.emit('note',({plant:'花盆被撞倒了，點一下把它扶好。',curtain:'窗簾被扯亂了，點一下整理。',catTree:'貓跳台歪倒了，點一下扶正。',lamp:'落地燈被撞倒了，點一下把它扶好。',litter:'貓砂灑到盆外了，點一下掃乾淨。',water:'飲水機旁濺了水，點一下擦乾。'})[kind]);return true;}
 cleanMess(kind){if(!MESS_PENALTIES[kind]||!this.messyObjects[kind])return false;const before=this.careScore(),clean=this.cleanliness;this.messyObjects[kind]=false;const delta=this.cleanliness-clean;this.emitStatFeedback([{id:'shared',stat:'cleanliness',delta}],before,this.careScore(),`RESET_${kind.toUpperCase()}`);this.checkDailyFullStats(['clean']);this.emit('clean-mess',({plant:'扶好花盆',curtain:'窗簾整理好了',catTree:'跳台扶正了',lamp:'扶好落地燈',litter:'掃乾淨灑出的貓砂',water:'擦乾飲水機旁的水漬'})[kind]+`　潔淨 +${delta}`,{id:kind});return true;}
 knockObject(kind){return this.setMess(kind);}
 resetObject(x,y){let kind=null;if(this.messyObjects.plant&&x>=-30&&x<=12&&y>=150&&y<=202)kind='plant';else if(this.messyObjects.curtain&&x>=5&&x<=128&&y>=18&&y<=157)kind='curtain';else if(this.messyObjects.catTree&&x>=-5&&x<=72&&y>=205&&y<=303)kind='catTree';else if(this.messyObjects.lamp&&x>=186&&x<=224&&y>=116&&y<=160)kind='lamp';else if(this.messyObjects.litter&&x>=205&&x<=270&&y>=265&&y<=306)kind='litter';else if(this.messyObjects.water&&x>=-22&&x<=45&&y>=268&&y<=310)kind='water';return kind?this.cleanMess(kind):false;}
 ...
 get interacting(){return !!this.rewardFood||!!this.wolf||!!this.close||this.cats.some(c=>c.pendingCall||c.goal==='close'||['turn-look','come-to-player'].includes(c.state)||c.resume?.goal==='close');}
 ...
 suppressRewardEvents(){
  ...
  if(this.critter){const chased=this.cats.find(c=>c.id===this.critter.target);if(chased?.goal==='critter'){chased.goal=null;this.set(chased,'sit',2);}this.critter=null;}
 }
 placeRewardFood(kind,x,y){
  ...
  const roll=this.random(),targets=roll<.4?['lumi']:roll<.8?['hoya']:['lumi','hoya'],targetCats=this.cats.filter(c=>targets.includes(c.id));for(const cat of this.cats){cat.pendingCall=false;cat.resume=null;cat.foodId=null;cat.yarnUntil=0;cat.sleeping=false;cat.napUntil=0;cat.windowIntent=null;cat.desiredLevel=0;if(!targets.includes(cat.id)&&!cat.jump){cat.goal=null;this.set(cat,'sit',2);}}
  for(const c of targetCats)c.goal='reward-food';
  const count=item.meows[0]+Math.floor(this.random()*(item.meows[1]-item.meows[0]+1));
  const meowTimes=Array.from({length:count},(_,i)=>Math.min(item.duration-.2,(i+1)*item.duration/(count+1)+(.5-this.random())*.2));
  ...
  this.rewardFood={id:++this.rewardSerial,kind,x,y,target:targets[0],targets,arrived:{},phase:'approach',age:0,nextHeart:0,meowTimes,meowIndex:0};
 ...
 updateRewardCat(c,dt){
  ...
  if(reward.phase==='approach'){
   const offset=targets.length>1?(targetIndex===0?-9:9):0,tx=clamp(reward.x+offset,this.floorBounds.left+8,this.floorBounds.right-8),ty=reward.y,dist=Math.hypot(c.x-tx,c.y-ty);c.goal='reward-food';c.tx=tx;c.ty=ty;
   if(dist>3){this.setRunningVisitor(c,tx,ty,dt,defs[c.id].speed*1.35);c.goal='reward-food';return true;}
   c.x=tx;c.y=ty;c.goal='reward-food';reward.arrived[c.id]=true;this.set(c,'sit',.4);if(!targets.every(id=>reward.arrived[id]))return true;reward.phase='eat';reward.age=0;reward.nextHeart=0;for(const id of targets){const eater=this.cats.find(cat=>cat.id===id);if(eater){this.set(eater,'eat');this.emit('eat',null,eater);}}
  }
  if(reward.phase==='eat'){
   const item=REWARD_FOODS[reward.kind];c.goal='reward-food';if(c.state!=='eat')this.set(c,'eat');if(c.id!==targets[0])return true;reward.age+=dt;
   if(reward.age>=reward.nextHeart){reward.nextHeart+=item.heartEvery;for(const id of targets)this.emit('reward-heart',null,this.cats.find(cat=>cat.id===id));}
   while(reward.meowIndex<reward.meowTimes.length&&reward.age>=reward.meowTimes[reward.meowIndex]){const id=targets[reward.meowIndex%targets.length];reward.meowIndex++;this.emit('meow',null,this.cats.find(cat=>cat.id===id));}
   if(Math.floor(reward.age*3)!==Math.floor((reward.age-dt)*3))for(const id of targets)this.emit('eat',null,this.cats.find(cat=>cat.id===id));
   if(reward.age>=item.duration){const name=item.name,names=targets.map(id=>defs[id].name).join(' 和 ');this.rewardFood=null;for(const id of targets){const eater=this.cats.find(cat=>cat.id===id);if(eater){eater.goal=null;this.set(eater,'sit',3);}}this.resetExternalCooldowns();this.emit('note',`${names} 吃完${name}，滿足地舔舔嘴巴。`,c);}
  }
  return true;
 }
 assignMeals(){
 ...
 applyOffline(since,now=Date.now()){if(!Number.isFinite(since)||!Number.isFinite(now)||now<=since)return;const seconds=Math.min(72*3600,(now-Math.max(since,this.offlineAppliedAt||since))/1000);if(seconds<=0)return;this.offlineAppliedAt=now;this.advanceTime(seconds,true);for(const c of this.cats)c.sleepiness=clamp(c.sleepiness+seconds/1800);const messes=this.evaluateOfflineChaos(since,now);if(seconds>=900)this.emit('note',messes?`你回來了。房間多了 ${messes} 處生活痕跡，點一下就能整理。`:'你回來了。牠們有點餓，也想你陪一會。');}
 companionReward(c,now=Date.now()){const daily=this.ensureDailyCoinRewards(now);if(daily.firstCompanion)return false;daily.firstCompanion=true;c.companionDay=daily.date;this.applyStatDelta({target:c,stat:'bond',requestedDelta:2},{source:'DAILY_COMPANIONSHIP_REWARD'});this.grantCoins(10,`今天第一次好好陪 ${defs[c.id].name}`,c);this.checkDailyFullStats(['bond'],{bondInteraction:true,now});return true;}
 ...
 if(!c.playMoodAwarded&&c.playEffectiveSeconds>=8){c.playMoodAwarded=true;this.applyStatChanges([{target:c,stat:'mood',requestedDelta:2},{target:c,stat:'bond',requestedDelta:2}],{source:'TOY_PLAY_SESSION'});this.checkDailyFullStats(['mood','bond'],{bondInteraction:true});}
 ...
 dropFood(x,y){if(this.interacting||this.foods.length>=2||this.platformAt(x,y)||x<this.floorBounds.left||x>this.floorBounds.right||y<205||y>this.floorBounds.bottom)return false;if(this.cats.every(c=>c.hunger>=20)){this.foods=[];for(const c of this.cats)c.foodId=null;this.emit('note','Lumi 和 Hoya 現在都不餓。');return false;}if(this.foods.some(b=>Math.hypot(x-b.x,y-b.y)<28)){this.emit('note','另一盤放遠一點，讓牠們有空間吃。');return false;}this.foods.push({id:++this.foodSerial,x,y,left:30});this.emit('note','香香的晚餐，等醒著的貓自己過來。');return true;}
 pet(distance,speed){const c=this.cats.find(c=>c.id===this.close);if(!c||distance<1)return false;if(['bite-warning','pet-satisfied'].includes(c.state))return false;if(speed>430){this.rejectPet('覺得你摸得太快了','fast');return false;}c.petGlow=1.2;c.petTotal+=distance;c.petSession=(c.petSession||0)+distance;this.set(c,'pet-happy',1.2);const milestone=24/defs[c.id].pat;if(c.petTotal>=milestone){c.petTotal=0;this.emit('purr','呼嚕嚕…就是這裡。',c);if((c.petMoodRewards||0)<2){c.petMoodRewards=(c.petMoodRewards||0)+1;this.applyStatDelta({target:c,stat:'mood',requestedDelta:2},{source:'PET_GENTLE_STROKE'});}if(this.time-c.rewardAt>8){this.grantActiveInteractionCoins(4,'溫柔摸摸',c);c.rewardAt=this.time;}this.checkDailyFullStats(['mood','bond'],{bondInteraction:true});}if(((c.petMoodRewards||0)>=2&&c.petSession>=180)||c.petSession>=700){this.finishPet(c);return true;}return c.petTotal===0;}
 advanceTime(seconds,offline=false){if(!Number.isFinite(seconds)||seconds<=0)return;const toiletSteps=[];for(const c of this.cats){const delayBefore=Math.max(0,c.toiletDelayRemaining||0);c.toiletDelayRemaining=Math.max(0,delayBefore-seconds);const b=c.timeBuckets||(c.timeBuckets=freshBuckets()),prefix=offline?'offline':'active',intervals=offline?{Hunger:3600,Mood:5400,Energy:7200}:{Hunger:120,Mood:180,Energy:240},floor=offline?8:0;for(const stat of ['Hunger','Mood','Energy']){if(stat==='Energy'&&c.sleeping)continue;const key=prefix+stat;b[key]=(b[key]||0)+seconds;const steps=Math.floor(b[key]/intervals[stat]);if(steps){b[key]-=steps*intervals[stat];this.passiveDelta(c,stat.toLowerCase(),-steps*2,floor);}}if(c.sleeping){b.sleepEnergy=(b.sleepEnergy||0)+seconds;const steps=Math.floor(b.sleepEnergy/20);if(steps){b.sleepEnergy-=steps*20;const restored=this.passiveDelta(c,'energy',steps*2,0);if(!offline&&restored>0)this.checkDailyFullStats(['energy']);}}b.toilet=(b.toilet||0)+seconds;const needs=Math.floor(b.toilet/900);if(needs)b.toilet-=needs*900;toiletSteps.push({c,needs,blocked:delayBefore>seconds});if(!offline){b.naturalPoop=(b.naturalPoop||0)+seconds;while(b.naturalPoop>=300){b.naturalPoop-=300;if(c.poopQueue===0&&!c.queuedToilet&&this.random()<.08)c.toiletNeed=100;}}b.anger=(b.anger||0)+seconds;const angerSteps=Math.floor(b.anger/10);if(angerSteps){b.anger-=angerSteps*10;c.anger=clamp(c.anger-angerSteps);}}
 ...
 if(this.rewardFood)this.suppressRewardEvents();else{this.updateVisitors(dt);this.updateCritter(dt);this.updateMosquito(dt);}
 this.updateGecko(dt);
 ...
 if(!this.rewardFood){this.updateMischief();this.updateYarnPlay();if(!this.wolf)this.updateBird(dt);this.updateSocial(dt);this.assignMeals();}
 ...
 if(c.timer<=0){this.applyStatDelta({target:'shared',stat:'water',requestedDelta:-1},{source:'CAT_DRINK'});c.nextDrink=this.time+45+this.random()*30;if(!this.messyObjects.water&&this.random()<.2)this.setMess('water');c.goal=null;this.target(c,75,282,'wander');}return true;
 }
...
root.CatRoom={World,defs,clamp,platforms,APP_VERSION,REWARD_FOODS,paintingNames,PAINTING_PRICE,getCleanlinessFromPoops,THREAT_EFFECTS,windowApproachPoint,windowJumpPoint,windowPerchLeft,windowPerchRight};if(typeof module!=='undefined')module.exports=root.CatRoom;
