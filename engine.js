...
class World{
 ...
 if(!this.plane&&!this.bird&&this.time>=(this.nextPlane??300)){this.nextPlane=this.time+55+this.random()*40;this.plane={age:0,direction:this.random()<.5?1:-1};this.bird=null;this.endSocial();this.emit('note','抬頭看，一架小飛機經過窗外。');}
 ...
 updateBird(dt){
 if(!this.bird){if(!this.outdoorNight&&!this.night&&this.time>=(this.nextBird??18)){this.bird={phase:'arrive',age:0,id:this.time,direction:this.random()<.5?1:-1};}return;}
 ...
 this.cleanliness=clamp(this.cleanliness-dt*.008);this.time+=dt;this.outdoorNight=hour>=18||hour<6;this.night=this.manualNight===true;
 ...
 cleanLitter(x,y){if(this.interacting||x<187||x>235||y<272||y>310)return false;if(this.cats.some(c=>c.state==='toilet')){this.emit('note','等牠上完廁所再清喔。');return false;}this.poops=0;this.cleanliness=100;this.emit('scoop','沙沙沙…貓砂盆乾淨了 ♥');return true;}
