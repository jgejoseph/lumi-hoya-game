...
const platforms=[{level:1,x:18,y:258,z:32,kind:'tree'},{level:2,x:39,y:258,z:80,kind:'tree'},{level:3,x:183,y:258,z:104,kind:'cabinet',halfWidth:42}];
...
class World{
 ...
 playYarn(){if(this.interacting||this.toy||this.night||this.current.sleeping)return false;this.endSocial();this.current.yarnUntil=this.time+12;return true;}
 updateYarnPlay(){
  if(this.time<(this.nextYarnPlay??75)||this.wolf||this.critter||this.plane||this.bird||this.social||this.close||this.toy||this.night||this.cats.some(c=>c.yarnUntil>this.time))return;
 ...
 updateCritter(dt){
  if(!this.critter){
   ...
   if(this.wolf||this.plane||this.bird||this.social||this.close||this.toy||this.night||this.cats.some(c=>c.yarnUntil>this.time)){this.nextCritter=this.time+15;return;}
   this.nextCritter=this.time+270+this.random()*60;
   ...
   this.critter={kind:this.random()<.5?'mouse':'roach',age:0,duration:8,x:this.floorBounds.left-16,y:this.floorBounds.bottom-16,target:target?.id||null};return;
  }
  const v=this.critter;v.age+=dt;v.x=this.floorBounds.left-16+(this.floorBounds.right-this.floorBounds.left+32)*Math.min(1,v.age/v.duration);
 ...
 chase(c,dt,toy){
  ...
  c.foodId=null;
  if(toy.level){c.desiredLevel=toy.level;if(c.level===toy.level){if(c.state!=='play')this.set(c,'play');if(this.playEnergy(c,dt)&&this.time-c.rewardAt>10){this.coins++;c.rewardAt=this.time;this.emit('toy',null,c);}}else if(c.level){this.jumpTo(c,toy.level);}else{if(c.goal!=='climb')this.climb(c,toy.level);this.move(c,dt,defs[c.id].speed*(c.id==='hoya'?2.5:1.65));}return;
  }
  if(c.level){c.desiredLevel=0;this.jumpTo(c,0);return;}
 ...
 this.random=random;this.cats=[freshCat('lumi'),freshCat('hoya')];this.selected='lumi';this.coins=150;this.water=15;this.night=false;this.outdoorNight=false;this.manualNight=null;this.close=null;this.events=[];this.foods=[];this.foodSerial=0;this.toy=null;this.time=0;this.poops=0;this.plantKnocked=false;this.lampKnocked=false;this.lastThreat='';this.nextMischief=70;this.nextPlane=45+random()*45;this.nextWolf=80+random()*65;this.nextYarnPlay=90;this.nextCritter=300;
 if(saved&&[1,2,3,4,5].includes(saved.version)){
  this.coins=Number.isFinite(saved.coins)?clamp(saved.coins,0,99999):150;this.water=saved.version>=4&&Number.isFinite(saved.water)?Math.round(clamp(saved.water,0,15)):15;this.selected=defs[saved.selected]?saved.selected:'lumi';if(typeof saved.manualNight==='boolean')this.manualNight=saved.manualNight;if(Number.isFinite(saved.lightsOffAt)&&saved.lightsOffAt<=Date.now())this.lightsOffAt=saved.lightsOffAt;
 ...
 updateVisitors(dt){
 if(this.time>=(this.nextWolf??420)&&!this.wolf){this.nextWolf=this.time+120+this.random()*90;const choices=['WOLF','DOG','BEAR'].filter(v=>v!==this.lastThreat),kind=choices[Math.floor(this.random()*choices.length)],target=this.random()<.5?'lumi':'hoya',names={WOLF:'狼',DOG:'狗狗',BEAR:'熊'};this.lastThreat=kind;this.wolf={kind,target,age:0,hits:0,flash:0};this.plane=null;this.bird=null;this.critter=null;this.endSocial();this.close=null;this.toy=null;this.applyThreat(kind,target);for(const c of this.cats){c.sleeping=false;c.pendingCall=false;c.resume=null;c.goal=null;this.set(c,'puffed',1.5);}this.emit(kind==='DOG'?'bark':kind==='BEAR'?'roar':'howl',`窗外有${names[kind]}！點牠 5 下，幫貓咪趕走牠。`);}
 if(this.hitFx){this.hitFx.age+=dt;if(this.hitFx.age>.32)this.hitFx=null;}if(this.wolf){this.wolf.age+=dt;this.wolf.flash=Math.max(0,this.wolf.flash-dt);if(this.wolf.age>=60)this.dismissWolf(false);return;}
 if(!this.plane&&!this.critter&&!this.social&&!this.cats.some(c=>c.yarnUntil>this.time)&&this.time>=(this.nextPlane??300)){this.nextPlane=this.time+55+this.random()*40;this.plane={age:0,direction:-1,variant:Math.floor(this.random()*3)};this.endSocial();this.emit('plane');this.emit('note','抬頭看，一架小飛機經過窗外。');}
 ...
 hitWolf(x,y){if(!this.wolf)return false;const dog=this.wolf.kind==='DOG';if(x<(dog?61:40)||x>(dog?97:99)||y<(dog?93:82)||y>126)return false;this.wolf.hits++;this.wolf.flash=.18;this.threatHitFeedback(x,y);if(this.wolf.hits>=5)this.dismissWolf(true);return true;}
 ...
 dismissWolf(defended=false){if(!this.wolf)return;const name={WOLF:'狼',DOG:'狗狗',BEAR:'熊'}[this.wolf.kind||'WOLF'];this.wolf=null;if(defended)this.defendCatsSuccess();this.emit('note',defended?'你保護了牠們！Lumi 和 Hoya 安心地走出來。':`${name}離開了，安全了。牠們探探頭，慢慢走出來。`);for(const c of this.cats){if(c.state==='hidden') {c.x=c.id==='lumi'?164:202;c.y=206;}c.goal=null;this.set(c,'sit',2);}}
 visitorCat(c,dt){
 ...
 if(this.plane&&!c.sleeping&&!this.interacting&&!this.toy&&!this.food&&!c.level&&c.anger<30){const x=c.id==='lumi'?58:88,y=215;if(Math.hypot(c.x-x,c.y-y)>2)this.setRunningVisitor(c,x,y,dt,defs[c.id].speed*1.5);else this.set(c,'watch-plane');return true;}
 if(c.goal==='critter'&&!this.critter){c.goal=null;this.set(c,'sit',2);}
 if(this.critter?.target===c.id&&!c.sleeping&&!c.level&&!c.jump&&c.toiletNeed<100&&!this.interacting){const v=this.critter;if(v.x>c.x+12){const x=Math.min(v.x-14,this.floorBounds.right-8),y=v.y;if(Math.hypot(c.x-x,c.y-y)>1)this.setRunningVisitor(c,x,y,dt,defs[c.id].speed*1.8);c.goal='critter';}else if(c.state!=='watch-critter')this.set(c,'watch-critter');return true;}return false;
 }
 ...
 updateBird(dt){
 if(!this.bird){if(!this.social&&!this.critter&&!this.night&&!this.cats.some(c=>c.yarnUntil>this.time)&&this.time>=(this.nextBird??18)){this.bird={phase:'arrive',age:0,id:this.time,direction:this.random()<.5?1:-1,variant:Math.floor(this.random()*3),kind:this.random()<.25?'butterfly':'bird'};}return;}
 ...
 else if(b.phase==='perch'){if(b.kind!=='butterfly'&&Math.floor(b.age/4)!==Math.floor((b.age-dt)/4))this.emit('chirp');if(b.age>=16){b.phase='leave';b.age=0;}}
 ...
 socialReady(){return !this.wolf&&!this.critter&&!this.plane&&!this.bird&&!this.close&&!this.food&&!this.toy&&!this.night&&!this.cats.some(c=>c.yarnUntil>this.time)&&this.cats.every(c=>!c.sleeping&&!c.level&&!c.jump&&c.energy>=6&&c.sleepiness<70&&c.anger<10&&['idle','sit','wander','walk','lounge'].includes(c.state)&&(!c.goal||c.goal==='wander'));}
 ...
 if(this.close||this.critter||this.food||this.toy||this.night||this.cats.some(c=>c.energy<6||c.sleepiness>=70||c.anger>=10)){this.endSocial();return;}
 if(s.phase==='approach'){
 ...
 toyAt(x,y){const p=this.platformAt(x,y);if(p)return {x:p.x,y:p.y,z:p.z,level:p.level,drawX:x,drawY:y};return x>=this.floorBounds.left&&x<=this.floorBounds.right&&y>=205&&y<=this.floorBounds.bottom?{x,y,level:0}:null;}
 available(level,c){return !this.cats.some(other=>other!==c&&(other.level===level||other.jump?.level===level));}
 climb(c,level){c.desiredLevel=level;const entry=level===3?platforms.find(p=>p.level===3):platforms[0];c.climbEntryLevel=level===3?3:1;this.target(c,entry.x,entry.y,'climb');}
 ...
 this.advanceTime(elapsedSeconds,false);this.time+=dt;this.outdoorNight=hour>=18||hour<6;this.night=this.manualNight===true;
 this.updateVisitors(dt);
 this.updateCritter(dt);
 if(!this.outdoorNight)this.meteor=null;
 else if(this.meteor){this.meteor.age+=dt;if(this.meteor.age>=.75)this.meteor=null;}
 else if(this.time>=(this.nextMeteor??60)){this.nextMeteor=this.time+90+this.random()*150;if(this.random()<.45)this.meteor={age:0,x:40+this.random()*25,y:44+this.random()*12};}
 this.updateMischief();
 this.updateYarnPlay();
 if(!this.wolf)this.updateBird(dt);
 this.updateSocial(dt);
 this.assignMeals();
 for(const c of this.cats){const d=defs[c.id];c.age+=dt;c.timer-=dt;c.petGlow=Math.max(0,c.petGlow-dt);c.sleepiness=clamp(c.sleepiness+dt*(c.sleeping?-1.1:.04));
 ...
 if(this.toy&&!this.night){c.yarnUntil=0;const offset=c.id==='lumi'?-6:6;this.chase(c,dt,{...this.toy,x:this.toy.x+offset});continue;}
 ...
 if(!this.toy&&!this.night&&c.yarnUntil>this.time&&c.energy>2&&!(c.playRestUntil>Date.now())){this.chase(c,dt,{x:this.yarn.x-10,y:this.yarn.y,level:0});continue;}
 if(this.toilet(c,dt))continue;if(!this.night&&this.feed(c,dt))continue;if(c.state==='eat'){this.set(c,'sit',2);c.goal=null;}
 if(c.level){
 if(this.night||c.energy<6||c.sleepiness>80){this.target(c,c.id==='lumi'?112:192,217,'bed');continue;}
 ...
 if((c.energy<6||c.sleepiness>80||this.night)&&c.goal!=='bed'){if(this.toy&&c.energy>6&&!this.night){}else{this.target(c,c.id==='lumi'?112:192,217,'bed');}}
 ...
 if(this.bird?.phase==='perch'&&!this.toy&&c.birdSeen!==this.bird.id&&c.energy>6&&['idle','sit','wander','walk','lounge'].includes(c.state)&&(!c.goal||c.goal==='wander')){c.birdSeen=this.bird.id;this.target(c,c.id==='lumi'?94:122,218,'bird');continue;}
 ...
 if(c.timer<=0){if(c.energy>6&&this.random()<.24&&this.available(1,c)){const choice=this.random();this.climb(c,choice<.2?3:choice<.6?1:2);}else if(this.random()<.18){this.target(c,95+this.random()*100,239+this.random()*47,'lounge');}else if(this.random()<.35){this.set(c,this.random()<.5?'sit':'idle',d.rest+this.random()*3);}else{this.set(c,'wander',.45);c.tx=30+this.random()*180;c.ty=228+this.random()*59;c.goal='wander';}}
 }
 }
 drink(c,dt){
 if(this.water<=0||this.toy||this.interacting||this.night){if(c.goal==='water'||c.state==='drink'){c.goal=null;this.set(c,'sit',2);}return false;}
 ...
 c.facing=-1;if(c.state!=='drink'){this.set(c,'drink',6);c.goal='water';this.emit('note',defs[c.id].name+' 正在舔水，咕嚕咕嚕。',c);}
