...
let started=false,last=0,elapsed=0,zoom=1,focus={x:120,y:160},drag=null,stroke=null,noteUntil=0,lastHud=0,lastSave=0,music=false,audio=null,bgmClock=0,bgmStep=0,particles=[],pointerId=null,audioMode=AUDIO_MODE.MUTED,activeThreatSessionId=0,threatAudioOwner=null,threatPlayFailedSession=0,dangerMediaPrimed=false,mosquitoVoice=null;
...
function room(t){
 ...
 if(world.outdoorNight){oval(92,49,7,7,'#f0ddb0');oval(95,46,6,6,'#4e6477');for(const [x,y]of [[32,43],[58,63],[104,78],[38,104]])rect(x,y,1,2,'#e6d8b7');}else{for(const [x,y,w,h]of [[38,35,2,5],[38,55,2,5],[26,46,5,2],[49,46,5,2],[29,37,3,3],[49,37,3,3],[29,56,3,3],[49,56,3,3]])rect(x,y,w,h,'#f1c76f');oval(40,48,8,8,'#f6d77e');oval(38,46,4,4,'#ffe7a0');rect(35,51,23,4,'#e2e4d2');rect(42,48,10,3,'#e2e4d2');rect(82,74,27,4,'#d7e0ce');oval(48,129,32,20,'#9eaf87');oval(100,127,26,18,'#899f7e');rect(20,136,94,6,'#a3b490');}
 drawBird();drawVisitors();
 rect(65,27,4,118,'#f4e9cd');rect(18,82,98,4,'#f4e9cd');
 rect(7,149,120,7,'#b09068');rect(11,143,112,7,'#ffefd1');
 if(world.isGoodnightMode){poly([[8,22],[67,22],[66,143],[8,147]],'#c9b596');poly([[67,22],[124,22],[127,147],[66,143]],'#c2ad90');for(const x of [17,30,43,56,78,91,104,117])line(x,25,x+(x<67?-4:4),142,'#aa9578');line(66,23,66,145,'#8f7e68',2);}else{poly([[11,22],[32,22],[29,87],[20,145],[8,149]],'#f4e6ce');poly([[104,22],[123,22],[127,149],[114,145],[109,86]],'#f4e6ce');line(20,29,14,142,'#dfcfb1');line(115,29,121,143,'#dfcfb1');}rect(6,18,122,4,'#9a8161');
 ...
 if(!world.outdoorNight&&!world.isGoodnightMode){g.globalAlpha=.17;poly([[30,139],[98,139],[155,242],[72,242]],'#fff9d8');line(71,139,118,242,'#c6ad83',3);g.globalAlpha=1;}
...
function drawVisitors(){
 ...
 if(world.plane){const colors=[['#e8e7d5','#839da0','#809da7'],['#e3e9dc','#86a48d','#789a91'],['#eee2e6','#b295a5','#929db1']][world.plane.variant%3],x=112-world.plane.age*5.4;g.save();g.translate(x,54);poly([[-9,0],[-6,-2],[7,-2],[10,0],[7,2],[-6,2]],colors[0]);poly([[1,-2],[5,-6],[8,-6],[6,0]],colors[1]);poly([[-1,1],[4,6],[7,6],[4,1]],colors[2]);poly([[-7,-1],[-9,-5],[-7,-5],[-4,0]],colors[1]);rect(-5,-1,2,1,'#607d8b');rect(-1,-1,2,1,'#607d8b');rect(3,-1,2,1,'#607d8b');g.restore();}
 if(world.wolf){
  ...
  if(kind==='DOG'){
   ...
   if(kind==='BEAR'){
    ...
    oval(70,126,25,22,fur);poly([[47,108],[45,84],[62,99]],fur);poly([[77,99],[94,84],[91,111]],fur);oval(70,109,22,17,fur);poly([[49,111],[69,120],[91,111],[81,125],[58,125]],'#c8c4b5');
...
function drawCritter(){const v=world.critter;if(!v)return;g.save();g.translate(v.x,v.y);g.scale(v.direction||1,1);const bob=Math.sin(v.age*24);if(v.kind==='mouse'){oval(0,-3,8,4,'#777b7d');oval(7,-5,4,3,'#929596');oval(5,-9,2,2,'#c49d9c');rect(8,-6,1,1,'#292c2b');rect(11,-5,1,1,'#bd8d8c');line(-7,-3,-14,-5+bob,'#8a8582');line(-14,-5+bob,-18,-3,'#8a8582');}else{oval(-5,-3,3,3,'#55463c');oval(0,-3,3,3,'#695347');oval(5,-3,4,3,'#55463c');rect(7,-4,1,1,'#2c2926');line(7,-5,11,-8+bob,'#55463c');line(7,-4,12,-3-bob,'#55463c');for(const dx of [-4,0,4]){line(dx,-2,dx-5,2+bob,'#55463c');line(dx,-2,dx+5,2-bob,'#55463c');}}g.restore();}
function drawMosquito(){const pop=world.mosquitoPop;if(pop){const p=pop.age/.45;g.save();g.globalAlpha=1-p;for(let i=0;i<6;i++){const a=i*Math.PI/3,d=3+p*9;rect(pop.x+Math.cos(a)*d,pop.y+Math.sin(a)*d,2,2,'#d79a63');}label('啪!',pop.x,pop.y-8-p*5,'#8b5f45',7);g.restore();}const m=world.mosquito;if(!m)return;g.save();g.translate(Math.round(m.x),Math.round(m.y));g.scale((m.direction||1)*.82,.82);const flap=Math.abs(Math.sin(m.age*28))*3;oval(-7,-5,7,2+flap,'#dce1d4dc');oval(-2,-7,5,2+flap,'#cbd5d0d8');line(-12,-5,-1,-3,'#8f9994',1);line(-7,-8,2,-5,'#8f9994',1);oval(-7,1,7,4,'#464b49');rect(-12,-2,4,6,'#303533');rect(-8,-3,3,8,'#6e736f');rect(-4,-2,3,6,'#292e2c');oval(0,-1,3,3,'#3b403d');oval(3,-4,3,3,'#f4f1e8');oval(6,-6,3,3,'#f4f1e8');rect(4,-5,1,1,'#292b2a');rect(7,-7,1,1,'#292b2a');line(8,-4,15,1,'#343735',2);for(const x of [-3,0,2]){line(x,1,x-3,9,'#464a47');line(x,1,x+6,7,'#464a47');}g.restore();}
function catTree(){
...
function drawCat(c,t){if(c.state==='hidden')return;const p=palette[c.id],walk=['walk','come-to-player','jump','chase-critter'].includes(c.state),sleep=c.sleeping||c.state==='social-rest'||c.state==='lounge',play=c.state==='play',rub=c.state==='rub',knead=c.state==='knead',front=['turn-look','come-to-player','pet-happy','chase-critter','watch-critter'].includes(c.state)||world.close===c.id;
 ...
 const shut=sleep||rub||knead||c.petGlow>0||(t%5.7<.14),angry=c.state==='angry'||c.state==='annoyed'||c.state==='bite-warning',tracksMosquito=world.mosquito&&!world.wolf&&!c.sleeping&&!c.jump&&!c.level&&!c.goal&&['idle','sit'].includes(c.state),lookScale=front?1:-c.facing,lookX=tracksMosquito?clamp(Math.round((world.mosquito.x-c.x)/35),-2,2)*lookScale:0,lookY=tracksMosquito?clamp(Math.round((world.mosquito.y-(feet+hy-2))/38),-1,1):0;
 for(const x of [-7,4]){if(shut){line(x,-1,x+3,-1,p.dark===palette.lumi.dark?'#312f30':'#796043');rect(x-1,-2,1,1,p.dark);}else{rect(x-1,-4,6,6,c.id==='lumi'?'#514846':'#79664c');rect(x,-4,4,5,p.eye);rect(x+1+lookX,-3+lookY,2,4,'#364445');rect(x,-4,2,2,'#fff6db');if(angry)line(x-1,-5,x+4,-2,p.dark,2);}}
...
function primeThreatBgm(){if(dangerMediaPrimed||!music||document.hidden||typeof dangerBgmMedia?.play!=='function')return;dangerMediaPrimed=true;try{dangerBgmMedia.muted=true;dangerBgmMedia.volume=0;const priming=dangerBgmMedia.play();priming?.then?.(()=>{if(audioMode!==AUDIO_MODE.THREAT){dangerBgmMedia.pause?.();dangerBgmMedia.currentTime=0;}dangerBgmMedia.muted=false;dangerBgmMedia.volume=.3;})?.catch?.(error=>{dangerMediaPrimed=false;dangerBgmMedia.muted=false;dangerBgmMedia.volume=.3;reportMediaFailure(error,AUDIO_MODE.THREAT,0);});}catch(error){dangerMediaPrimed=false;dangerBgmMedia.muted=false;reportMediaFailure(error,AUDIO_MODE.THREAT,0);}}
function beginThreatAudio(threat){const sessionId=++activeThreatSessionId;threatAudioOwner=threat;threatPlayFailedSession=0;audioMode=AUDIO_MODE.THREAT;try{bgmMedia?.pause?.();dangerBgmMedia?.pause?.();dangerBgmMedia.currentTime=0;dangerBgmMedia.muted=false;dangerBgmMedia.volume=.3;}catch{}playMedia(dangerBgmMedia,AUDIO_MODE.THREAT,sessionId);}
function endThreatAudio(sessionId){if(sessionId!==activeThreatSessionId)return;try{dangerBgmMedia?.pause?.();dangerBgmMedia.currentTime=0;}catch{}threatAudioOwner=null;threatPlayFailedSession=0;if(music&&started&&!document.hidden){audioMode=AUDIO_MODE.NORMAL;bgmMedia.volume=.3;playMedia(bgmMedia,AUDIO_MODE.NORMAL);}else audioMode=AUDIO_MODE.MUTED;}
function syncAudioOwnership(retry=false){if(!music||!started||document.hidden){audioMode=AUDIO_MODE.MUTED;return;}if(world.wolf){if(threatAudioOwner!==world.wolf||audioMode!==AUDIO_MODE.THREAT)beginThreatAudio(world.wolf);else if(retry&&threatPlayFailedSession===activeThreatSessionId){threatPlayFailedSession=0;playMedia(dangerBgmMedia,AUDIO_MODE.THREAT,activeThreatSessionId);}return;}if(threatAudioOwner){endThreatAudio(activeThreatSessionId);return;}if(audioMode!==AUDIO_MODE.NORMAL){try{dangerBgmMedia?.pause?.();dangerBgmMedia.currentTime=0;}catch{}audioMode=AUDIO_MODE.NORMAL;bgmMedia.volume=.3;playMedia(bgmMedia,AUDIO_MODE.NORMAL);}else if(retry&&bgmMedia?.paused)playMedia(bgmMedia,AUDIO_MODE.NORMAL);}
function stopAudio(){audioPaused=true;audioMode=AUDIO_MODE.MUTED;activeThreatSessionId++;threatAudioOwner=null;threatPlayFailedSession=0;stopMosquitoBuzz();try{for(const media of [bgmMedia,dangerBgmMedia])media?.pause?.();dangerBgmMedia.currentTime=0;navigator.vibrate?.(0);}catch{}for(const osc of voices){try{osc.stop();}catch{}}voices.clear();if(audio?.state==='running')audio.suspend().catch(()=>{});}
...
function stopMosquitoBuzz(){if(!mosquitoVoice)return;for(const node of [mosquitoVoice.osc,mosquitoVoice.lfo]){voices.delete(node);try{node.stop();}catch{}}try{mosquitoVoice.osc.disconnect();mosquitoVoice.lfo.disconnect();mosquitoVoice.gain.disconnect();}catch{}mosquitoVoice=null;}
function syncMosquitoBuzz(){const active=world.mosquito&&!world.wolf&&!world.isGoodnightMode&&music&&!audioPaused&&!document.hidden&&audio?.state==='running';if(!active){stopMosquitoBuzz();return;}if(mosquitoVoice)return;const osc=audio.createOscillator(),lfo=audio.createOscillator(),gain=audio.createGain(),mod=audio.createGain(),t=audio.currentTime;osc.type='sawtooth';osc.frequency.value=165;lfo.type='sine';lfo.frequency.value=7.2;gain.gain.value=.008;mod.gain.value=.0025;lfo.connect(mod);mod.connect(gain.gain);osc.connect(gain);gain.connect(audio.destination);mosquitoVoice={osc,lfo,gain};voices.add(osc);voices.add(lfo);osc.start(t);lfo.start(t);}
function sound(type,id=world.selected){if(type==='angry')haptic('bite',[30,45,55],.8);if(type==='toy')haptic('toy',[10,35,10],.35);if(type==='land')haptic('land',12,.3);if(type==='scoop')haptic('scoop',8,.07);if(type==='thump')haptic('wolf',[20,25,30],.1);if(type==='swat')haptic('swat',[24,20,38],.55);if(!music||audioPaused||!audio||audio.state!=='running')return;if(type==='purr'){if(elapsed-lastPurr<.5)return;lastPurr=elapsed;}if(type==='plane'){tone(88,8,'triangle',.018,0,68);tone(132,8,'sine',.011,0,96);}if(type==='howl'){tone(320,1.4,'triangle',.065,0,510);tone(510,1.2,'sine',.055,.3,390);tone(390,.75,'sine',.045,1.2,560);}if(type==='bark'){tone(310,.16,'square',.055,0,190);tone(360,.18,'square',.05,.23,205);}if(type==='roar'){tone(105,1.2,'sawtooth',.075,0,62);tone(155,1.05,'triangle',.055,.12,82);}if(type==='squeak'){tone(900,.13,'triangle',.078,0,1720);tone(1290,.1,'square',.045,.03,2050);tone(1020,.13,'triangle',.072,.2,1800);tone(1430,.09,'square',.04,.23,2160);}if(type==='drink'){tone(430,.09,'sine',.038,0,260);tone(570,.07,'triangle',.032,.13,330);tone(390,.1,'sine',.03,.28,230);}if(type==='dog-love'){tone(620,.12,'sine',.022);tone(820,.18,'sine',.018,.12);}if(type==='swat'){tone(115,.15,'square',.12,0,42);tone(720,.055,'triangle',.085,0,180);tone(1480,.04,'square',.045,.045,390);}if(type==='scoop'){for(let i=0;i<14;i++)tone(700+Math.random()*1800,.055,'triangle',.045,i*.045,180);}if(type==='thump')tone(150,.18,'triangle',.11,0,45);if(type==='jump')tone(220,.13,'sine',.027,0,440);if(type==='chirp'){tone(1900,.12,'sine',.021,0,2700);tone(2500,.08,'sine',.016,.15,2100);}if(type==='land')tone(95,.09,'sine',.026,0,55);if(type==='meow'){meowVoice(id);}if(type==='purr'){for(let i=0;i<7;i++){tone(86+i%2*12,.13,'triangle',.055,i*.09,72);tone(170,.12,'sine',.018,i*.09,145);}}if(type==='eat'){tone(190,.045,'triangle',.02,0,90);tone(260,.05,'triangle',.015,.06,110);}if(type==='toy'){tone(1800,.06,'sine',.025);tone(2300,.08,'sine',.016,.07);}if(type==='coin'){tone(880,.14,'sine',.035);tone(1320,.25,'sine',.025,.12);}if(type==='angry'){tone(155,.3,'sawtooth',.018,0,80);}}
...
$('start').onclick=()=>{started=true;try{music=localStorage.getItem('lh-muted')!=='true';}catch{music=true;}soundHud();syncAudioOwnership();primeThreatBgm();unlock().then(ready=>{if(ready){bgmClock=0;sound('meow');}});$('intro').classList.add('leaving');setTimeout(()=>$('intro').hidden=true,750);notify('歡迎回家。點點 Lumi 或 Hoya 打個招呼。');hud();};
$('music').onclick=()=>{music=!music;if(music){syncAudioOwnership();primeThreatBgm();unlock().then(ready=>{if(ready){bgmClock=0;sound('meow');}});}else stopAudio();soundHud();try{localStorage.setItem('lh-muted',String(!music));}catch{}};
...
document.addEventListener('pointerdown',()=>{if(music){syncAudioOwnership(true);primeThreatBgm();unlock();}},{capture:true});
...
$('night').onclick=()=>{world.toggleNight();syncAudioOwnership();save();hud();};
...
function frame(t){const rawDt=last?Math.max(0,(t-last)/1000):0,dt=Math.min(.05,rawDt);last=t;if(!document.hidden){elapsed+=rawDt;if(started)world.update(dt,new Date().getHours(),rawDt);else world.outdoorNight=new Date().getHours()>=18||new Date().getHours()<6;syncAudioOwnership();syncMosquitoBuzz();audioTick(dt);for(const event of world.events.splice(0)){sound(event.type,event.id);if(event.text)notify(event.text);if(event.type==='stat-feedback'){showStatFeedback(event);hud();}if(event.type==='coin')particles.push({x:200,y:40,life:1.4,kind:'coin'});if(event.type==='dog-love')particles.push({x:79+(Math.random()-.5)*10,y:98,life:1.4,kind:'heart'});}render(dt);if(elapsed>noteUntil)$('note').classList.remove('visible');if(elapsed-lastHud>.2){hud();lastHud=elapsed;}if(started&&elapsed-lastSave>5){save();lastSave=elapsed;}}else stopMosquitoBuzz();requestAnimationFrame(frame);}
