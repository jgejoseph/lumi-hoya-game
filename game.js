...
(()=>{
'use strict';
const {World,defs,clamp,platforms}=CatRoom,$=id=>document.getElementById(id),canvas=$('room'),g=canvas.getContext('2d');
...
function haptic(kind,pattern,interval=.3){if(document.hidden||!navigator.vibrate||elapsed-(hapticTimes[kind]??-1)<interval)return;hapticTimes[kind]=elapsed;try{navigator.vibrate(pattern);}catch{}}
...
function room(t){
 rect(0,0,240,320,'#eadbc0');rect(0,0,240,191,'#e9ddc5');rect(0,0,240,5,'#d5c3a4');
 for(let x=10;x<240;x+=22){rect(x,0,1,190,'#e2d4b9');for(let y=17;y<185;y+=24){rect(x+9,y,2,2,'#d7cbae');rect(x+8,y+1,4,1,'#d7cbae');}}
 rect(0,190,240,8,'#a78a68');rect(0,190,240,2,'#c7ab86');rect(0,198,240,122,'#d4b087');
 for(let y=201;y<320;y+=19){rect(0,y,240,1,'#b99673');for(let x=(Math.floor(y/19)%2)*39;x<240;x+=78){rect(x,y,1,19,'#bea07c');rect(x+7,y+5,22,1,'#ddba92');rect(x+37,y+12,15,1,'#c6a27c');}}
 ...
 if(world.outdoorNight){oval(83,54,7,7,'#f0ddb0');oval(86,51,6,6,'#4e6477');for(const [x,y]of [[40,50],[61,68],[92,83],[37,93]])rect(x,y,1,2,'#e6d8b7');}else{rect(35,52,23,4,'#e2e4d2');rect(42,49,10,3,'#e2e4d2');rect(74,72,24,4,'#d7e0ce');oval(49,114,28,22,'#9eaf87');oval(92,113,22,17,'#899f7e');rect(28,120,75,6,'#a3b490');}
...
function drawVisitors(){
 ...
 if(world.plane){const x=112-world.plane.age*5.4,y=54;rect(x-13,y,25,3,'#faf0d4');poly([[x-3,y+1],[x+8,y-7],[x+11,y-7],[x+4,y+2]],'#d68763');poly([[x-2,y+2],[x+8,y+9],[x+11,y+9],[x+5,y+1]],'#d68763');poly([[x+9,y],[x+14,y-5],[x+16,y-5],[x+15,y+2]],'#789ba8');rect(x-11,y,3,1,'#608194');}
...
function drawBird(){
 const b=world.bird;if(!b)return;let x=82,y=80;
 // The bird lives outside: clip before drawing, then let the frame and curtains occlude it.
 g.save();g.beginPath();g.rect(28,36,75,90);g.clip();
 if(b.phase==='arrive'){const p=Math.min(1,b.age/1.8);x=14+68*p;y=43+37*p-Math.sin(p*Math.PI)*12;}
 if(b.phase==='startled'){x+=Math.sin(b.age*55)*2;y-=Math.abs(Math.sin(b.age*25))*4;}if(b.phase==='leave'){const p=Math.min(1,b.age/(b.fright?.8:1.6));x+=42*p;y-=44*p;}
 const flying=b.phase!=='perch';oval(x,y-4,6,4,'#238ebc');oval(x,y-2,4,2,'#ffd66b');oval(x+4,y-8,4,4,'#39b5d6');rect(x+5,y-9,2,2,'#263e59');rect(x+5,y-9,1,1,'#fff8de');poly([[x+7,y-8],[x+11,y-6],[x+7,y-6]],'#ee9c3c');poly([[x-5,y-4],[x-11,y-9],[x-10,y-2]],'#2564a0');
 if(flying){const flap=Math.sin(b.age*24)*7;poly([[x-3,y-5],[x-7,y-8+flap],[x+2,y-5+flap]],'#59cde0');}else{oval(x-2,y-5,3,2,'#2564a0');line(x-2,y-1,x-2,y+1,'#9a653e');line(x+2,y-1,x+2,y+1,'#9a653e');}g.restore();
}
function catTree(){
...
function render(dt){const target=world.close?3.3:1;zoom+=(target-zoom)*Math.min(1,dt*5);const closeCat=world.cats.find(c=>c.id===world.close),fx=closeCat?closeCat.x:120,fy=closeCat?closeCat.y-19:160;focus.x+=(fx-focus.x)*Math.min(1,dt*5);focus.y+=(fy-focus.y)*Math.min(1,dt*5);
 g.imageSmoothingEnabled=false;g.save();g.translate(120,160);g.scale(zoom,zoom);g.translate(-focus.x,-focus.y);room(elapsed);drawLitter();for(const food of world.foods)bowl(food.x,food.y);const layers=world.cats.map(c=>({depth:c.y+(c.level||c.jump?.level?1:0),draw:()=>drawCat(c,elapsed)}));layers.push({depth:258,draw:catTree});layers.sort((a,b)=>a.depth-b.depth).forEach(layer=>layer.draw());if(world.social?.phase==='active')label('♡',141,222-(world.social.age%2.5)*5,'#c58f86',9);if(world.toy)feather(world.toy.drawX??world.toy.x,world.toy.drawY??world.toy.y,elapsed);g.restore();
 ...
 if(drag?.type==='scoop'){line(drag.x,drag.y-17,drag.x,drag.y,'#947a59',3);rect(drag.x-6,drag.y,13,9,'#8eaaa5');for(let i=-3;i<5;i+=3)line(drag.x+i,drag.y+2,drag.x+i,drag.y+7,'#e8dec5');}if(drag?.inside&&drag.type==='food')bowl(drag.x,drag.y);
}
...
function stopAudio(){audioPaused=true;try{navigator.vibrate?.(0);}catch{}for(const osc of voices){try{osc.stop();}catch{}}voices.clear();if(audio?.state==='running')audio.suspend().catch(()=>{});}
function soundHud(){$('music').setAttribute('aria-pressed',music);$('music').setAttribute('aria-label',music?'關閉全部聲音':'開啟全部聲音');}
function unlock(){if(!music||document.hidden)return;audioPaused=false;if(!audio){try{audio=new(window.AudioContext||window.webkitAudioContext)();}catch{return;}}if(audio.state==='suspended')audio.resume().catch(()=>{});}
...
function sound(type,id=world.selected){if(music&&!audioPaused&&audio?.state==='suspended'){audio.resume().then(()=>{if(music&&!audioPaused&&audio.state==='running')sound(type,id);}).catch(()=>{});return;}if(type==='angry')haptic('bite',[30,45,55],.8);if(type==='toy')haptic('toy',[10,35,10],.35);if(type==='land')haptic('land',12,.3);if(type==='purr'){if(elapsed-lastPurr<.5)return;lastPurr=elapsed;}if(type==='howl'){tone(230,2.4,'sawtooth',.075,0,460);tone(460,2.6,'sine',.08,.25,280);tone(280,1.8,'triangle',.06,1.8,170);}if(type==='scoop'){for(let i=0;i<14;i++)tone(700+Math.random()*1800,.055,'triangle',.035,i*.045,180);}if(type==='thump'){tone(150,.18,'triangle',.09,0,45);haptic('wolf',[20,25,30],.1);}if(type==='jump')tone(220,.13,'sine',.018,0,440);if(type==='chirp'){tone(1900,.12,'sine',.012,0,2700);tone(2500,.08,'sine',.009,.15,2100);}if(type==='land')tone(95,.09,'sine',.018,0,55);if(type==='meow'){meowVoice(id);}if(type==='purr'){for(let i=0;i<7;i++){tone(86+i%2*12,.13,'triangle',.045,i*.09,72);tone(170,.12,'sine',.014,i*.09,145);}}if(type==='eat'){tone(190,.045,'triangle',.012,0,90);tone(260,.05,'triangle',.009,.06,110);}if(type==='toy'){tone(1800,.06,'sine',.015);tone(2300,.08,'sine',.009,.07);}if(type==='coin'){tone(880,.14,'sine',.025);tone(1320,.25,'sine',.017,.12);}if(type==='angry'){tone(155,.3,'sawtooth',.009,0,80);}}
...
function audioTick(dt){if(!music||!started||!audio||audio.state!=='running')return;bgmClock-=dt;if(bgmClock<=0){bgmClock=.6;const notes=[261.63,329.63,392,493.88,440,392,329.63,293.66,246.94,293.66,392,440,392,329.63,293.66,0];const section=Math.floor(bgmStep/16)%8,transpose=[1,1.12246,.8909,1,1.2599,1.12246,.8909,1][section];const index=section%2?15-bgmStep%16:bgmStep%16;const f=notes[index]*transpose;if(f)tone(f,.95,'sine',.009);if(bgmStep%4===0){tone(bgmStep%16<8?130.81:146.83,2.1,'sine',.012);tone(bgmStep%16<8?196:220,1.8,'sine',.006,.1);}bgmStep=(bgmStep+1)%128;}}
function screenPoint(e){const r=canvas.getBoundingClientRect(),scale=Math.min(r.width/240,r.height/320),left=r.left+(r.width-240*scale)/2,top=r.top+(r.height-320*scale)/2;return {x:(e.clientX-left)/scale,y:(e.clientY-top)/scale};}
function toWorld(p){return {x:(p.x-120)/zoom+focus.x,y:(p.y-160)/zoom+focus.y};}
...
for(const type of ['food','toy','scoop']){const source=$(type);source.addEventListener('pointerdown',e=>dragStart(e,type));source.addEventListener('pointermove',e=>{if(!drag||e.pointerId!==drag.id)return;if(world.interacting){drag=null;world.toy=null;return;}e.preventDefault();const p=screenPoint(e);Object.assign(drag,p,{inside:p.x>=24&&p.x<=216&&p.y>=213&&p.y<=299});if(type==='toy'){world.toy=world.toyAt(p.x,p.y);drag.inside=!!world.toy;if(world.toy&&elapsed-lastBell>.35){lastBell=elapsed;sound('toy');}}});source.addEventListener('pointerup',e=>{if(!drag||e.pointerId!==drag.id)return;const p=screenPoint(e);if(type==='scoop')world.cleanLitter(p.x,p.y);if(type==='food'){if(world.dropFood(p.x,p.y))sound('toy');else if(world.foods.length>=2)notify('房間最多放兩盤。');else if(world.coins>=5)notify('放在地毯附近的地板上就可以了。');}world.toy=null;drag=null;save();hud();});for(const event of ['pointercancel','lostpointercapture'])source.addEventListener(event,()=>{if(drag?.source===source){drag=null;world.toy=null;hud();}});}
$('start').onclick=()=>{started=true;try{music=localStorage.getItem('lh-muted')!=='true';}catch{music=true;}soundHud();unlock();$('intro').classList.add('leaving');setTimeout(()=>$('intro').hidden=true,750);sound('meow');notify('歡迎回家。點點 Lumi 或 Hoya 打個招呼。');hud();};
$('music').onclick=()=>{music=!music;if(music)unlock();else stopAudio();soundHud();try{localStorage.setItem('lh-muted',String(!music));}catch{}};
