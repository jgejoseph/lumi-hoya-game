...
function renderKnowledge(){
 const ticker=$('knowledge-ticker'),items=knowledge?.visible||[];if(!items.length){ticker.hidden=true;return;}
 knowledgeIndex=Math.min(knowledgeIndex,items.length-1);const item=items[knowledgeIndex];ticker.hidden=false;ticker.setAttribute('aria-expanded',knowledgeExpanded);
 $('knowledge-text').textContent=knowledgeExpanded?`${item.title}　${item.text}`:item.text;ticker.setAttribute('aria-label',`${item.title}：${item.text}`);
}
...
function drawVisitors(){
 ...
 if(world.plane){const direction=world.plane.direction||1,x=direction>0?8+world.plane.age*5.4:112-world.plane.age*5.4,y=54;g.save();g.translate(x,y);g.scale(direction,1);rect(-13,0,25,3,'#faf0d4');poly([[-3,1],[8,-7],[11,-7],[4,2]],'#d68763');poly([[-2,2],[8,9],[11,9],[5,1]],'#d68763');poly([[9,0],[14,-5],[16,-5],[15,2]],'#789ba8');rect(-11,0,3,1,'#608194');g.restore();}
...
function drawBird(){
 const b=world.bird;if(!b)return;const direction=b.direction||1;let x=82,y=80;
 ...
 g.save();g.translate(x,y);g.scale(direction,1);const flying=b.phase!=='perch';oval(0,-4,6,4,'#238ebc');oval(0,-2,4,2,'#ffd66b');oval(4,-8,4,4,'#39b5d6');rect(5,-9,2,2,'#263e59');rect(5,-9,1,1,'#fff8de');poly([[7,-8],[11,-6],[7,-6]],'#ee9c3c');poly([[-5,-4],[-11,-9],[-10,-2]],'#2564a0');
 if(flying){const flap=Math.sin(b.age*24)*7;poly([[-3,-5],[-7,-8+flap],[2,-5+flap]],'#59cde0');}else{oval(-2,-5,3,2,'#2564a0');line(-2,-1,-2,1,'#9a653e');line(2,-1,2,1,'#9a653e');}g.restore();g.restore();
}
function catTree(){
...
$('knowledge-ticker').onclick=()=>{knowledgeExpanded=!knowledgeExpanded;renderKnowledge();};
